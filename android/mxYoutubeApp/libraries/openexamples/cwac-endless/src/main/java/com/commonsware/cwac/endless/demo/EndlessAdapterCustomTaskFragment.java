 ...
 * 
 * Calling setRunInBackground(false) allows you to launch your own AsyncTask with a listener callback, rather than using
 * the built in cacheInBackground functionality.
 * 
 * This is useful if you have existing AsyncTask(s) written to fetch data in a background thread, and don't want
 * EndlessAdapter launching that in yet another background thread.
 */
public class EndlessAdapterCustomTaskFragment extends ListFragment {
	...
	class DemoAdapter extends EndlessAdapter implements IItemsReadyListener {
		...
		DemoAdapter(ArrayList<Integer> list) {
			super(new ArrayAdapter<Integer>(getActivity(), R.layout.row, android.R.id.text1, list));
	...
	class FetchDataTask extends AsyncTask<Void, Void, ArrayList<Integer>> {
		...
		/*
		 * The point from where to start counting. In a real life scenario this could be a pagination number
		 */
		int startPoint;
