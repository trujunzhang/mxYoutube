...
public class DashVodRendererBuilder implements ExoplayerWrapper.RendererBuilder,
	...
	/**
	 * Constant indicating that the Widevine security level is unknown. This constant must be an int instead of part of
	 * an enum because it has significance in the Exoplayer library. The Widevine security levels are documented here:
	 * https://source.android.com/devices/drm.html#security
	 */
	private static final int SECURITY_LEVEL_UNKNOWN = -1;

	/**
	 * Constant indicating that first Widevine security level. This constant must be an int instead of part of an enum
	 * because it has significance in the Exoplayer library. The Widevine security levels are documented here:
	 * https://source.android.com/devices/drm.html#security
	 */
	private static final int SECURITY_LEVEL_1 = 1;

	/**
	 * Constant indicating that third Widevine security level. This constant must be an int instead of part of an enum
	 * because it has significance in the Exoplayer library. The Widevine security levels are documented here:
	 * https://source.android.com/devices/drm.html#security
	 */
	...
	/**
	 * The maximum duration in milliseconds for which this video renderer can attempt to seamlessly join an ongoing
	 * playback.
	 */
	public static final int ALLOWED_JOINING_TIME_MS = 5000;

	/**
	 * The maximum number of frames that can be dropped between invocations of onDroppedFrames(int, long).
	 */
	...
	/**
	 * Contains the {@link com.google.android.exoplayer.ExoPlayer} which will use this renderer builder to play a video.
	 */
	...
	public void onManifest(String contentId, MediaPresentationDescription manifest) {
		...
		System.out.println("djzhang: mimeType = " + mimeType);

		// Pick the chunk source based on the MIME type of the content.
		if (mimeType.equals(MimeTypes.VIDEO_MP4)) {
			videoChunkSource = new DashMp4ChunkSource(videoDataSource, new AdaptiveEvaluator(bandwidthMeter),
			...
			// definition streams.
			videoRepresentations = getSdRepresentations(videoRepresentations);
			videoChunkSource = new DashWebmChunkSource(videoDataSource, new AdaptiveEvaluator(bandwidthMeter),
					videoRepresentations);
		} else {
		...
		ChunkSampleSource videoSampleSource = new ChunkSampleSource(videoChunkSource, loadControl,
				VIDEO_BUFFER_SEGMENTS * BUFFER_SEGMENT_SIZE, true, mainHandler, player, ExoplayerWrapper.TYPE_VIDEO);
		MediaCodecVideoTrackRenderer videoRenderer = new MediaCodecVideoTrackRenderer(videoSampleSource,
				drmSessionManager, true, MediaCodec.VIDEO_SCALING_MODE_SCALE_TO_FIT, ALLOWED_JOINING_TIME_MS,
				mainHandler, player, MAX_DROPPED_FRAME_COUNT_TO_NOTIFY);
		...
		if (audioRepresentationsList.isEmpty()) {
			...
			SampleSource audioSampleSource = new ChunkSampleSource(audioChunkSource, loadControl, AUDIO_BUFFER_SEGMENTS
					* BUFFER_SEGMENT_SIZE, true, mainHandler, player, ExoplayerWrapper.TYPE_AUDIO);
			audioRenderer = new MediaCodecAudioTrackRenderer(audioSampleSource, drmSessionManager, true, mainHandler,
					player);
		}
	...
	/**
	 * Iterate through each of the representations and return an array of all representations which are standard
	 * definition (less than 720 x 1280).
	 *
	 * @param representations
	 *            A set of representations which include the format widths and heights.
	 */
	...
	private static class V18Compat {
		...
		/**
		 *
		 * Return a pair where the first element is the DRM session manager and the second element is whether the
		 * security level is 1.
		 *
		 * @param player
		 *            The {@link ExoplayerWrapper} which will receive this renderer builder and use it to play the
		 *            video.
		 * @param drmCallback
		 *            Performs {@link android.media.MediaDrm} key and provisioning requests.
		...
		public static Pair<DrmSessionManager, Boolean> getDrmSessionManagerData(ExoplayerWrapper player,
			...
			StreamingDrmSessionManager streamingDrmSessionManager = new StreamingDrmSessionManager(
					ExoplayerUtil.WIDEVINE_UUID, player.getPlaybackLooper(), drmCallback, player.getMainHandler(),
					player);
		...
		/**
		 * Reads the security level string from the session manager and returns the appropriate constant - one of
		 * {@link #SECURITY_LEVEL_1}, {@link #SECURITY_LEVEL_3}, or {@link #SECURITY_LEVEL_UNKNOWN}. The Widevine
		 * security levels are documented here https://source.android.com/devices/drm.html#security
		 *
		 * @param sessionManager
		 *            Manages the DRM session.
		 */
