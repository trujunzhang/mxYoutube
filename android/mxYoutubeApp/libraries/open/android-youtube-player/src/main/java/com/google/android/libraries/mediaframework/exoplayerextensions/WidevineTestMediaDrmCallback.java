...
public class WidevineTestMediaDrmCallback implements MediaDrmCallback {
	...
	@Override
	public byte[] executeProvisionRequest(UUID uuid, ProvisionRequest request) throws ClientProtocolException,
			IOException {
		String url = request.getDefaultUrl() + "&signedRequest=" + new String(request.getData());
		byte[] bytes = ExoplayerUtil.executePost(url, null, null);
	...
	public byte[] executeKeyRequest(UUID uuid, KeyRequest request) throws IOException {
		...
		byte[] bytes = ExoplayerUtil.executePost(url, request.getData(), null);
		String result = new String(bytes);
		return bytes;
	}
