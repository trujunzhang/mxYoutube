 ...
 *
 * <p>
 * 2) a bottom chrome which contains a seek bar, fullscreen button, and text views indicating the current time and total
 ...
 *
 * <p>
 * 6) Adding an action button by providing an image, a content description, and a click handler. If there is enough
 * room, the action buttons will be displayed on the right of the top chrome. If there is NOT enough room, an overflow
 * button will be displayed. When the overflow button is clicked, a dialog box listing the content descriptions for the
 * action buttons is displayed. The action is then triggered by selecting it from the dialog box.
 *
 * <p>
...
public class PlaybackControlLayer implements Layer, PlayerControlCallback {
	...
	/**
	 * In order to imbue the {@link PlaybackControlLayer} with the ability make the player fullscreen, a
	 * {@link PlaybackControlLayer.FullscreenCallback} must be assigned to it. The
	 * {@link PlaybackControlLayer.FullscreenCallback} implementation is responsible for hiding/showing the other views
	 ...
	 *
	 * <p>
	 * 1) The top chrome which contains the logo, title, and action buttons.
	 *
	 * <p>
	 * 2) The bottom chrome which contains the play/pause button, seek bar, and fullscreen buttons.
	 *
	 * <p>
	...
	/**
	 * Displayed in the left of the top chrome - shows a logo. This is optional; if no image is provided, then no logo
	 * will be displayed.
	 */
	...
	/**
	 * Whether the play button has been pressed and the video should be playing. We include this variable because the
	 * video may pause when buffering must occur. Although the video will usually resume automatically when the
	 * buffering is complete, there are instances (i.e. ad playback), where it will not resume automatically. So, if we
	 * detect that the video is paused after buffering and should be playing, we can resume it programmatically.
	 */
	...
	/**
	 * Contains the logo, video title, and other actions button. It can be tinted with a color for branding.
	 */
	private RelativeLayout topChrome;

	/**
	 * This is the root view which contains all other views that make up the playback control layer. It can be tinted by
	 * setting the chrome color.
	 */
	private FrameLayout playbackControlRootView;

	/**
	 * Contains the seek bar, current time, end time, and fullscreen button. The background can be tinted with a color
	 * for branding.
	 */
	 ...
	 *
	 * @param activity
	 *            The activity that contains the video player.
	 * @param icon
	...
	/**
	 * Add the playback control layer back to the container. The playback controls disappear after timeout milliseconds.
	 *
	 * @param timeout
	 *            Hide the view after timeout milliseconds. If timeout == 0, then the playback controls will not
	 *            disappear unless their container is tapped again.
	 */
	...
	/**
	 * The action buttons are displayed in the top right of the video player. If the player is in portrait mode, then
	 * display an overflow button which displays a dialog window containing the possible actions. If the player is in
	 * landscape, then display the images for the actions in the top right of the video player.
	 */
	public void updateActionButtons() {
		...
		if (isFullscreen) {
			...
			FrameLayout.LayoutParams layoutParams = new FrameLayout.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT,
					ViewGroup.LayoutParams.WRAP_CONTENT);
			int margin = 5 * getLayerManager().getActivity().getResources().getDisplayMetrics().densityDpi;
			layoutParams.setMargins(margin, 0, margin, 0);
