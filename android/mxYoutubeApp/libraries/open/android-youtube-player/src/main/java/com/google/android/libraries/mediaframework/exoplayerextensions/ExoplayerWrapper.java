...
/**
 * A wrapper around {@link ExoPlayer} that provides a higher level interface. It can be prepared with one of a number of
 * {@link RendererBuilder} classes to suit different use cases (e.g. DASH, SmoothStreaming and so on).
 */
public class ExoplayerWrapper implements ExoPlayer.Listener, ChunkSampleSource.EventListener,
	...
	public interface RendererBuilderCallback {
		 ...
		 *
		 * @param trackNames
		 *            The names of the available tracks, indexed by {@link ExoplayerWrapper} TYPE_* constants. May be
		 *            null if the track names are unknown. An individual element may be null if the track names are
		 *            unknown for the corresponding type.
		 * @param multiTrackSources
		 *            Sources capable of switching between multiple available tracks, indexed by
		 *            {@link ExoplayerWrapper} TYPE_* constants. May be null if there are no types with multiple tracks.
		 *            An individual element may be null if it does not have multiple tracks.
		 * @param renderers
		 *            Renderers indexed by {@link ExoplayerWrapper} TYPE_* constants. An individual element may be null
		 *            if there do not exist tracks of the corresponding type.
		 */
	...
	public interface PlaybackListener {
		 ...
		 *
		 * @param playWhenReady
		 *            Whether the player should play as soon as it is set up.
		 * @param playbackState
		 *            The state of Exoplayer instance.
		 */
	 ...
	 * <p>
	 * These errors are not visible to the user, and hence this listener is provided for informational purposes only.
	 * Note however that an internal error may cause a fatal error if the player fails to recover. If this happens,
	 * {@link PlaybackListener#onError(Exception)} will be invoked.
	 *
	 * <p>
	 * Implementing an {@link InternalErrorListener} is a good way to identify why ExoPlayer may be behaving in an
	 * undesired way.
	 */
	public interface InternalErrorListener {
		 ...
		 *
		 * @param sourceId
		 *            The id of the source of the video.
		 * @param e
		 *            The error.
		 */
	...
	public interface InfoListener {
		 ...
		 *
		 * @param formatId
		 *            The new format of the video.
		 * @param trigger
		 *            The reason for a chunk being selected.
		 * @param mediaTimeMs
		 ...
		 * @param elapsed
		 *            The number of milliseconds in which the frames were dropped.
		 */
		 ...
		 *
		 * @param elapsedMs
		 *            The duration of the sampling period in milliseconds.
		 * @param bytes
		 *            The number of bytes received during the sampling period.
		 * @param bandwidthEstimate
		 *            The estimated bandwidth in bytes/sec, or
		 *            {@link com.google.android.exoplayer.upstream.DefaultBandwidthMeter #NO_ESTIMATE} if no estimate is
		 *            available. Note that this estimate is typically derived from more information than {@code bytes}
		 *            and {@code elapsedMs}.
		 */
		 ...
		 *            Whether this is the first time we are loading from the source.
		 * @param mediaStartTimeMs
		 *            The time point of the media where we start loading.
		 * @param mediaEndTimeMs
		 *            The time point of the media where we end loading.
		 * @param totalBytes
		 *            The total number of bytes loaded.
		 */
		void onLoadStarted(int sourceId, String formatId, int trigger, boolean isInitialization, int mediaStartTimeMs,
	...
	/**
	 * Exoplayer renderers are managed in an array (the array representation is used throughout the Exoplayer library).
	 *
	 * <p>
	...
	/**
	 * This variable must be an int, not part of an enum because it has significance within the Exoplayer library.
	 */
	...
	/**
	 * Responsible for loading the data from the source, processing it, and providing byte streams. By modifying the
	 * renderer builder, we can support different video formats like DASH, MP4, and SmoothStreaming.
	 */
	...
	/**
	 * Listeners are notified when the video size changes, when the underlying player's state changes, or when an error
	 * occurs.
	 */
	...
	/**
	 * States are idle, prepared, buffering, ready, or ended. This is an integer (instead of an enum) because the
	 * Exoplayer library uses integers.
	 */
	...
	/**
	 * Sources capable of switching between multiple available tracks, indexed by ExoplayerWrapper INDEX_* constants.
	 * May be null if there are no types with multiple tracks. An individual element may be null if it does not have
	 * multiple tracks.
	 */
	private MultiTrackChunkSource[] multiTrackSources;

	/**
	 * The names of the available tracks, indexed by DemoPlayer INDEX_* constants. May be null if the track names are
	 * unknown. An individual element may be null if the track names are unknown for the corresponding type.
	 */
	...
	/**
	 * Respond to changes in media format changes, load events, bandwidth estimates, and dropped frames.
	 */
	private InfoListener infoListener;

	/**
	 * @param rendererBuilder
	 *            Responsible for loading the data from the source, processing it, and providing byte streams. By
	 *            modifying the renderer builder, we can support different video formats like DASH, MP4, and
	 *            SmoothStreaming.
	 */
	...
	/**
	 * Returns the player control which can be used to play, pause, seek, get elapsed time, and get elapsed duration.
	 */
	 ...
	 *
	 * @param listener
	 *            The listener which responds to the error events.
	 */
	 ...
	 *
	 * @param listener
	 *            Listens to media format changes, bandwidth samples, load events, and dropped frames.
	 */
	 ...
	 *
	 * @param listener
	 *            The listener which can respond to text like subtitles and captions.
	 */
	 ...
	 *
	 * <p>
	 * In order to clear the surface, a message must be sent to the playback thread. To guarantee that this message is
	 * delivered, Exoplayer uses a blocking operation. Therefore, this method is blocking.
	 */
	 ...
	 *
	 * @param type
	 *            The index indicating the type of video (ex {@link #TYPE_VIDEO})
	 */
...
/**
   ...
   */
	 ...
	 *            The index indicating the type of video (ex {@link #TYPE_VIDEO}).
	 * @param state
	 *            Either {@link #PRIMARY_TRACK} or {@link #DISABLED_TRACK).
	 */
	 ...
	 *
	 * @param trackNames
	 *            The names of the available tracks, indexed by {@link ExoplayerWrapper} TYPE_* constants. May be null
	 *            if the track names are unknown. An individual element may be null if the track names are unknown for
	 *            the corresponding type.
	 * @param multiTrackSources
	 *            Sources capable of switching between multiple available tracks, indexed by {@link ExoplayerWrapper}
	 *            TYPE_* constants. May be null if there are no types with multiple tracks. An individual element may be
	 *            null if it does not have multiple tracks.
	 * @param renderers
	 *            Renderers indexed by {@link ExoplayerWrapper} TYPE_* constants. An individual element may be null if
	 *            there do not exist tracks of the corresponding type.
	 */
	 ...
	 * 
	 * @param playWhenReady
	 *            If true, playback will start as soon as the player is setup. If false, it must be started
	 *            programmatically.
	 */
	 ...
	 * 
	 * @param positionMs
	 *            A number of milliseconds after the start of the video.
	 */
	...
	/**
	 * Returns the position of the seek head in the number of milliseconds after the start of the video.
	 */
	...
	public void onLoadStarted(int sourceId, String formatId, int trigger, boolean isInitialization,
		...
		if (infoListener != null) {
			infoListener.onLoadStarted(sourceId, formatId, trigger, isInitialization, mediaStartTimeMs, mediaEndTimeMs,
					totalBytes);
		}
	}
	...
	@Override
	public void onUpstreamDiscarded(int sourceId, int mediaStartTimeMs, int mediaEndTimeMs, long totalBytes) {
		// Do nothing.
	}

	@Override
	public void onDownstreamDiscarded(int sourceId, int mediaStartTimeMs, int mediaEndTimeMs, long totalBytes) {
		// Do nothing.
	}

	/**
	 * If either playback state or the play when ready values have changed, notify all the playback listeners.
	 */
	 ...
	 *            If true, then message sent to the underlying playback thread is guaranteed to be delivered. However,
	 ...
	 * 
	 * @param type
	 *            The index of the video type (it must be one of the TYPE_* constants).
	 * @param allowRendererEnable
	 *            If true, the renderer is enabled.
	 */
