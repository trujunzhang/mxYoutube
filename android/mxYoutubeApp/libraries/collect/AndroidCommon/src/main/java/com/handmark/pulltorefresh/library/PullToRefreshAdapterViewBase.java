...
public abstract class PullToRefreshAdapterViewBase<T extends AbsListView> extends PullToRefreshBase<T> implements
	...
	private boolean isFirstItemVisible() {
		...
		if (null == adapter || adapter.isEmpty()) {
			...
			/**
			 * This check should really just be: mRefreshableView.getFirstVisiblePosition() == 0, but PtRListView
			 * internally use a HeaderView which messes the positions up. For now we'll just add one to account for it
	...
	private boolean isLastItemVisible() {
		...
		if (null == adapter || adapter.isEmpty()) {
			...
			/**
			 * This check should really just be: lastVisiblePosition == lastItemPosition, but PtRListView internally
			 * uses a FooterView which messes the positions up. For me we'll just subtract one to account for it and
			 * rely on the inner condition which checks getBottom().
			 */
