...
public class SlideHolder extends FrameLayout {
	...
	/**
	 * @param allow
	 *            - if false, SlideHolder won't react to swiping gestures (but still will be able to work by manually
	 *            invoking mathods)
	 */
	...
	/**
	 * @param dispatch
	 *            - if true, in open state SlideHolder will dispatch touch events to main layout (in other words - it
	 *            will be clickable)
	 */
	...
	/**
	 * @param opened
	 *            - if true, SlideHolder will always be in opened state (which means that swiping won't work)
	 */
	...
	private boolean isSlideAllowed() {
		return (mDirection * mEndOffset > 0 && mDirection * mOffset < mDirection * mEndOffset && mDirection * mOffset >= mDirection
				* mStartOffset)
				|| (mEndOffset == 0 && mDirection * mOffset > mDirection * mEndOffset && mDirection * mOffset <= mDirection
						* mStartOffset);
	}
