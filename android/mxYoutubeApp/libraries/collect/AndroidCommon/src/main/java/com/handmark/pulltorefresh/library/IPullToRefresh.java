...
public interface IPullToRefresh<T extends View> {
	...
	/**
	 * Demos the Pull-to-Refresh functionality to the user so that they are aware it is there. This could be useful when
	 * the user first opens your app, etc. The animation will only happen if the Refresh View (ListView, ScrollView,
	 * etc) is in a state where a Pull-to-Refresh could occur by a user's touch gesture (i.e. scrolled to the
	 * top/bottom).
	 * 
	...
	/**
	 * Returns a proxy object which allows you to call methods on all of the LoadingLayouts (the Views which show when
	 * Pulling/Refreshing).
	 * <p/>
	...
	/**
	 * Returns a proxy object which allows you to call methods on the LoadingLayouts (the Views which show when
	 * Pulling/Refreshing). The actual LoadingLayout(s) which will be affected, are chosen by the parameters you give.
	 * <p/>
	 * You should not keep the result of this method any longer than you need it.
	 * 
	 * @param includeStart
	...
	/**
	 * Gets whether Overscroll support is enabled. This is different to Android's standard Overscroll support (the
	 * edge-glow) which is available from GINGERBREAD onwards
	 * 
	...
	/**
	 * Sets whether Overscroll support is enabled. This is different to Android's standard Overscroll support (the
	 * edge-glow). This setting only takes effect when running on device with Android v2.3 or greater.
	 * 
	 * @param enabled
	...
	/**
	 * Sets the Widget to be in the refresh state. The UI will be updated to show the 'Refreshing' view, and be scrolled
	 * to show such.
	 */
	...
	/**
	 * By default the Widget disables scrolling on the Refreshable View while refreshing. This method can change this
	 * behaviour.
	 * 
